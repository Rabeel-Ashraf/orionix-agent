...
class MCPToolClient(ToolService):
    ...
    async def list_tools(self) -> Sequence[Tool]:
        try:
            ...
            return [mcp_tool_to_orionix_agent_tool(t) for t in tools]
        except Exception as e:
    ...
    async def read_tool(self, name: str) -> Tool:
        try:
            ...
            return mcp_tool_to_orionix_agent_tool(tool)
        except Exception as e:
            raise ToolError(str(e))
...
# Partial mapping of mcp types to orionix_agent types using fields "type" and "format"
...
def mcp_tool_to_orionix_agent_tool(mcp_tool: McpTool) -> Tool:
    parameters = {}
    for param in mcp_tool.inputSchema["properties"]:
        parameters[param] = (
            mcp_parameter_to_orionix_agent_parameter(param, mcp_tool.inputSchema),
            ToolParameterOptions(),
        )
    tool = Tool(
        name=mcp_tool.name,
...
def mcp_parameter_to_orionix_agent_parameter(
