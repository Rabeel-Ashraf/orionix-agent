...
from orionix_agent.adapters.loggers.websocket import WebSocketLogger
from orionix_agent.api.authorization import (
    AuthorizationPolicy,
    DevelopmentAuthorizationPolicy,
    ProductionAuthorizationPolicy,
)
from orionix_agent.core.capabilities import CapabilityStore, CapabilityVectorStore
from orionix_agent.core.common import IdGenerator
from orionix_agent.core.engines.alpha import message_generator
from orionix_agent.core.engines.alpha.guideline_matching.generic import (
...
from orionix_agent.core.engines.alpha.guideline_matching.generic.disambiguation_batch import (
    DisambiguationGuidelineMatchesSchema,
)
from orionix_agent.core.engines.alpha.guideline_matching.generic.journey_node_selection_batch import (
    JourneyNodeSelectionSchema,
)
from orionix_agent.core.engines.alpha.guideline_matching.generic_guideline_matching_strategy_resolver import (
    GenericGuidelineMatchingStrategyResolver,
)
from orionix_agent.core.engines.alpha.guideline_matching.generic.guideline_previously_applied_actionable_batch import (
...
from orionix_agent.core.engines.alpha.guideline_matching.generic.response_analysis_batch import (
    GenericResponseAnalysisSchema,
    GenericResponseAnalysisBatch,
    GenericResponseAnalysisShot,
)
from orionix_agent.core.engines.alpha.guideline_matching.generic.guideline_actionable_batch import (
    GenericActionableGuidelineMatchesSchema,
    GenericActionableGuidelineMatching,
    GenericActionableGuidelineGuidelineMatchingShot,
)
from orionix_agent.core.engines.alpha.guideline_matching.generic.guideline_previously_applied_actionable_customer_dependent_batch import (
...
from orionix_agent.core.engines.alpha.guideline_matching.generic import observational_batch
from orionix_agent.core.engines.alpha.guideline_matching.generic.observational_batch import (
    GenericObservationalGuidelineMatchesSchema,
    ObservationalGuidelineMatching,
    GenericObservationalGuidelineMatchingShot,
)
from orionix_agent.core.engines.alpha.guideline_matching.guideline_matcher import (
    GuidelineMatcher,
    GuidelineMatchingStrategyResolver,
    ResponseAnalysisBatch,
)
from orionix_agent.core.engines.alpha.hooks import EngineHooks
from orionix_agent.core.engines.alpha.optimization_policy import (
    BasicOptimizationPolicy,
    OptimizationPolicy,
)
from orionix_agent.core.engines.alpha.perceived_performance_policy import (
    BasicPerceivedPerformancePolicy,
    PerceivedPerformancePolicy,
)
from orionix_agent.core.engines.alpha.relational_guideline_resolver import RelationalGuidelineResolver
from orionix_agent.core.engines.alpha.tool_calling.overlapping_tools_batch import (
    OverlappingToolsBatchSchema,
)
from orionix_agent.core.engines.alpha.canned_response_generator import (
...
from orionix_agent.core.journey_guideline_projection import JourneyGuidelineProjection
from orionix_agent.core.services.indexing.guideline_agent_intention_proposer import (
    AgentIntentionProposerSchema,
)
from orionix_agent.core.journeys import JourneyStore, JourneyVectorStore
from orionix_agent.core.persistence.vector_database import VectorDatabase
from orionix_agent.core.services.indexing.customer_dependent_action_detector import (
    CustomerDependentActionDetector,
    CustomerDependentActionSchema,
)
from orionix_agent.core.services.indexing.guideline_action_proposer import (
    GuidelineActionProposer,
    GuidelineActionPropositionSchema,
)
from orionix_agent.core.services.indexing.guideline_continuous_proposer import (
    GuidelineContinuousProposer,
    GuidelineContinuousPropositionSchema,
)
from orionix_agent.core.services.indexing.relative_action_proposer import RelativeActionSchema
from orionix_agent.core.services.indexing.tool_running_action_detector import (
    ToolRunningActionDetector,
    ToolRunningActionSchema,
)
from orionix_agent.core.canned_responses import CannedResponseStore, CannedResponseVectorStore
from orionix_agent.core.nlp.service import NLPService
from orionix_agent.core.persistence.common import MigrationRequired, ServerOutdated
from orionix_agent.core.shots import ShotCollection
from orionix_agent.core.tags import TagDocumentStore, TagStore
from orionix_agent.api.app import create_api_app, ASGIApplication
from orionix_agent.core.background_tasks import BackgroundTaskService
from orionix_agent.core.contextual_correlator import ContextualCorrelator
from orionix_agent.core.agents import AgentDocumentStore, AgentStore
from orionix_agent.core.context_variables import ContextVariableDocumentStore, ContextVariableStore
from orionix_agent.core.emission.event_publisher import EventPublisherFactory
from orionix_agent.core.emissions import EventEmitterFactory
from orionix_agent.core.customers import CustomerDocumentStore, CustomerStore
from orionix_agent.core.evaluations import (
    EvaluationListener,
...
from orionix_agent.core.entity_cq import EntityQueries, EntityCommands
from orionix_agent.core.relationships import (
    RelationshipDocumentStore,
    RelationshipStore,
)
from orionix_agent.core.guidelines import (
    GuidelineDocumentStore,
    GuidelineStore,
)
from orionix_agent.adapters.db.json_file import JSONFileDocumentDatabase
from orionix_agent.core.nlp.embedding import (
    BasicEmbeddingCache,
    Embedder,
    EmbedderFactory,
    EmbeddingCache,
...
from orionix_agent.core.nlp.generation import SchematicGenerator
from orionix_agent.core.persistence.data_collection import DataCollectingSchematicGenerator
from orionix_agent.core.services.tools.service_registry import (
    ServiceRegistry,
    ServiceDocumentRegistry,
)
from orionix_agent.core.sessions import (
    PollingSessionListener,
    SessionDocumentStore,
    SessionListener,
    SessionStore,
)
from orionix_agent.core.glossary import GlossaryStore, GlossaryVectorStore
from orionix_agent.core.engines.alpha.engine import AlphaEngine
from orionix_agent.core.guideline_tool_associations import (
    GuidelineToolAssociationDocumentStore,
    GuidelineToolAssociationStore,
)
from orionix_agent.core.engines.alpha.tool_calling import single_tool_batch
from orionix_agent.core.engines.alpha.tool_calling.default_tool_call_batcher import DefaultToolCallBatcher
from orionix_agent.core.engines.alpha.tool_calling.single_tool_batch import (
    SingleToolBatchSchema,
    SingleToolBatchShot,
)
from orionix_agent.core.engines.alpha.tool_calling.tool_caller import ToolCallBatcher, ToolCaller


from orionix_agent.core.engines.alpha.message_generator import (
    MessageGenerator,
    MessageGeneratorShot,
    MessageSchema,
)
from orionix_agent.core.engines.alpha.tool_event_generator import ToolEventGenerator
from orionix_agent.core.engines.types import Engine
from orionix_agent.core.services.indexing.behavioral_change_evaluation import (
    BehavioralChangeEvaluator,
    LegacyBehavioralChangeEvaluator,
)
from orionix_agent.core.services.indexing.coherence_checker import (
    CoherenceChecker,
    ConditionsEntailmentTestsSchema,
    ActionsContradictionTestsSchema,
)
from orionix_agent.core.services.indexing.guideline_connection_proposer import (
    GuidelineConnectionProposer,
    GuidelineConnectionPropositionsSchema,
)
from orionix_agent.core.loggers import CompositeLogger, FileLogger, LogLevel, Logger
from orionix_agent.core.application import Application
from orionix_agent.core.version import VERSION


DEFAULT_PORT = 8800
SERVER_ADDRESS = "https://localhost"
CONFIG_FILE_PATH = Path("orionix_agent.toml")

DEFAULT_NLP_SERVICE = "openai"

DEFAULT_HOME_DIR = "runtime-data" if Path("runtime-data").exists() else "orionix_agent-data"
...
LOGGER = FileLogger(PARLANT_HOME_DIR / "orionix_agent.log", CORRELATOR, LogLevel.INFO)
...
def load_nlp_service(name: str, extra_name: str, class_name: str, module_path: str) -> NLPService:
    try:
        ...
        LOGGER.critical(
            f"{name} support is not installed. Please install it with: pip install orionix_agent[{extra_name}]."
        )
        sys.exit(1)
...
def load_anthropic() -> NLPService:
    return load_nlp_service(
        "Anthropic", "anthropic", "AnthropicService", "orionix_agent.adapters.nlp.anthropic_service"
    )


def load_aws() -> NLPService:
    return load_nlp_service("AWS", "aws", "BedrockService", "orionix_agent.adapters.nlp.aws_service")


def load_azure() -> NLPService:
    from orionix_agent.adapters.nlp.azure_service import AzureService
...
def load_cerebras() -> NLPService:
    return load_nlp_service(
        "Cerebras", "cerebras", "CerebrasService", "orionix_agent.adapters.nlp.cerebras_service"
    )


def load_deepseek() -> NLPService:
    return load_nlp_service(
        "DeepSeek", "deepseek", "DeepSeekService", "orionix_agent.adapters.nlp.deepseek_service"
    )


def load_gemini() -> NLPService:
    return load_nlp_service(
        "Gemini", "gemini", "GeminiService", "orionix_agent.adapters.nlp.gemini_service"
    )


def load_openai() -> NLPService:
    from orionix_agent.adapters.nlp.openai_service import OpenAIService
...
def load_together() -> NLPService:
    return load_nlp_service(
        "Together.ai", "together", "TogetherService", "orionix_agent.adapters.nlp.together_service"
    )


def load_litellm() -> NLPService:
    return load_nlp_service(
        "LiteLLM", "litellm", "LiteLLMService", "orionix_agent.adapters.nlp.litellm_service"
    )
...
async def get_module_list_from_config() -> list[str]:
    if CONFIG_FILE_PATH.exists():
        ...
        return list(config.get("orionix_agent", {}).get("modules", []))
    ...
    try:
        ...
        async def get_shared_chroma_db() -> VectorDatabase:
            ...
            if shared_chroma_db is None:
                from orionix_agent.adapters.vector_db.chroma import ChromaDatabase
...
async def check_required_schema_migrations() -> None:
    from orionix_agent.bin.prepare_migration import detect_required_migrations
    from orionix_agent.adapters.vector_db.chroma import ChromaDatabase
    ...
    if await detect_required_migrations(JSONFileDocumentDatabase, ChromaDatabase):
        die(
            ...
            "`orionix_agent-prepare-migration` and then re-run the server with `--migrate`."
        )
...
@asynccontextmanager
async def start_orionix_agent(params: StartupParameters) -> AsyncIterator[Container]:
    LOGGER.set_level(
        params.log_level
    ...
    if "PARLANT_HOME" not in os.environ and DEFAULT_HOME_DIR == "runtime-data":
        ...
        LOGGER.warning(
            "Please rename 'runtime-data' to 'orionix_agent-data' to avoid this warning in the future."
        )
...
def main() -> None:
    ...
    @click.option(
        "--anthropic",
        is_flag=True,
        help="Run with Anthropic. The environment variable ANTHROPIC_API_KEY must be set and install the extra package orionix_agent[anthropic].",
        default=False,
    )
    @click.option(
        "--aws",
    ...
    Also, install the extra package orionix_agent[aws]."""
        ),
        default=False,
    )
    @click.option(
    ...
    @click.option(
        "--cerebras",
        is_flag=True,
        help="Run with Cerebras. The environment variable CEREBRAS_API_KEY must be set and install the extra package orionix_agent[cerebras].",
        default=False,
    )
    @click.option(
        "--deepseek",
        is_flag=True,
        help="Run with DeepSeek. You must set the DEEPSEEK_API_KEY environment variable and install the extra package orionix_agent[deepseek].",
        default=False,
    )
    @click.option(
        "--gemini",
        is_flag=True,
        help="Run with Gemini. The environment variable GEMINI_API_KEY must be set and install the extra package orionix_agent[gemini].",
        default=False,
    )
    @click.option(
        "--together",
        is_flag=True,
        help="Run with Together AI. The environment variable TOGETHER_API_KEY must be set and install the extra package orionix_agent[together].",
        default=False,
    )
    @click.option(
        "--litellm",
        ...
        help="""Run with LiteLLM. The following environment variables must be set: LITELLM_PROVIDER_MODEL_NAME, LITELLM_PROVIDER_API_KEY.
                ...
                set them and install the extra package orionix_agent[litellm].""",
        default=False,
    )
    @click.option(
        "--log-level",
    ...
    @click.option(
        ...
        help=(
            ...
            "If orionix_agent.toml exists in the working directory, any additional modules specified "
    ...
    def run(
        ...
        async def start() -> None:
            async with start_orionix_agent(ctx.obj):
                pass
    ...
    def enable_module(name: str) -> None:
        ...
        if not CONFIG_FILE_PATH.exists():
            CONFIG_FILE_PATH.write_text(toml.dumps({"orionix_agent": {"modules": [name]}}))
        else:
            content = toml.loads(CONFIG_FILE_PATH.read_text())
            enabled_modules = cast(list[str], content["orionix_agent"]["modules"])
