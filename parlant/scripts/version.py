...
def set_package_version(version: str, package: Package) -> None:
    ...
    with open(project_file, "w") as file:
        ...
        project_file_content = re.sub(
            f'\norionix_agent-(.+?) = "{current_version}"\n',
            f'\norionix_agent-\\1 = "{version}"\n',
            project_file_content,
        )
...
def update_version_variable_in_code(version: str) -> None:
    server_package = next(p for p in get_packages() if p.name == "orionix_agent")
    version_file: Path = server_package.path / "src/orionix_agent/core/version.py"
