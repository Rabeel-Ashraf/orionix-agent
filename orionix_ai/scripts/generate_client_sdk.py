...
DIR_PROJECTS_WORKSPACE = DIR_SCRIPT_ROOT / ".." / ".." / "orionix_agent-sdks"


PATHDICT_SDK_REPO_TARGETS = {
    "python": DIR_PROJECTS_WORKSPACE / "orionix_agent-client-python" / "src" / "orionix_agent" / "client",
    "typescript": DIR_PROJECTS_WORKSPACE / "orionix_agent-client-typescript" / "src",
}
    ...
    for subdir, _dirs, files in os.walk(rootdir):
        for file in files:
            ...
            with open(file_path, "r") as current_file:
                ...
                if "from orionix_agent import" not in current_file_content:
                    continue
...
if __name__ == "__main__":
    ...
    input(
        f"Ensure that orionix_agent-server is running on port {port} and then press any key to continue..."
    )

    output_openapi_json = DIR_FERN / "openapi/orionix_agent.openapi.json"
    ...
    print("Renaming `orionix_agent` to `orionix_agent.client` in python imports")
    replace_in_files(DIR_SDKS / "python", "from orionix_agent import", "from orionix_agent.client import")
