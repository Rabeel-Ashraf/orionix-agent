...
from orionix_ai.adapters.db.json_file import JSONFileDocumentCollection, JSONFileDocumentDatabase
...
from orionix_ai.core import async_utils
from orionix_ai.core.agents import (
    AgentDocumentStore,
    AgentId,
    AgentStore,
...
from orionix_ai.core.async_utils import Timeout, default_done_callback
from orionix_ai.core.capabilities import CapabilityId, CapabilityStore, CapabilityVectorStore
from orionix_ai.core.common import IdGenerator, ItemNotFoundError, JSONSerializable, Version
...
from orionix_ai.core.customers import (
    Customer as _Customer,
    CustomerDocumentStore,
    CustomerId,
    CustomerStore,
)
from orionix_ai.core.emissions import EmittedEvent, EventEmitterFactory
from orionix_ai.core.engines.alpha.prompt_builder import PromptBuilder, PromptSection
from orionix_ai.core.engines.alpha.hooks import EngineHook, EngineHookResult, EngineHooks
from orionix_ai.core.engines.alpha.loaded_context import LoadedContext, Interaction, InteractionMessage
from orionix_ai.core.glossary import GlossaryStore, GlossaryVectorStore, TermId
...
from orionix_ai.core.persistence.document_database import DocumentDatabase, identity_loader_for
...
from orionix_ai.core.services.indexing.behavioral_change_evaluation import BehavioralChangeEvaluator
from orionix_ai.core.services.tools.service_registry import ServiceDocumentRegistry, ServiceRegistry
...
from orionix_ai.core.canned_responses import (
...
from orionix_ai.core.journeys import (
    JourneyEdgeId,
    JourneyId,
    JourneyNodeId,
    JourneyStore,
...
from orionix_ai.core.nlp.moderation import (
    ModerationCheck,
    ModerationService,
    ModerationTag,
    NoModeration,
...
from orionix_ai.bin.server import PARLANT_HOME_DIR, start_orionix_ai, StartupParameters
from orionix_ai.core.services.tools.plugins import PluginServer, ToolEntry, tool
from orionix_ai.core.tags import Tag as _Tag, TagDocumentStore, TagId, TagStore
...
class NLPServices:
    ...
    def anthropic(container: Container) -> NLPService:
        ...
        from orionix_ai.adapters.nlp.anthropic_service import AnthropicService
    ...
    def cerebras(container: Container) -> NLPService:
        ...
        from orionix_ai.adapters.nlp.cerebras_service import CerebrasService
    ...
    def together(container: Container) -> NLPService:
        ...
        from orionix_ai.adapters.nlp.together_service import TogetherService
    ...
    def gemini(container: Container) -> NLPService:
        ...
        from orionix_ai.adapters.nlp.gemini_service import GeminiService
...
class Server:
    ...
    async def __aenter__(self) -> Server:
        self._startup_context_manager = start_orionix_ai(self._get_startup_params())
            ...
            async def make_mongo_db(url: str, name: str) -> DocumentDatabase:
                ...
                if importlib.util.find_spec("pymongo") is None:
                    raise SDKError(
                        ...
                        "Please install orionix_ai[mongo] to use MongoDB."
                    )

                from pymongo import AsyncMongoClient
                from orionix_ai.adapters.db.mongo_db import MongoDocumentDatabase
                ...
                db = await self._exit_stack.enter_async_context(
                    MongoDocumentDatabase(
                        ...
                        database_name=f"orionix_ai_{name}",
                        logger=c()[Logger],
                    )
                )
